...
|| 通道3 | GPIO4 | 7脚  | 已配置 | 通用 GPIO，/dev/gpiochip4 line 4 |
|| 通道4 | GPIO7 | 26脚 | 已配置 | 通用 GPIO，/dev/gpiochip4 line 7 |
...
**挑战**：
- 10 kHz 验证 → 100 kHz 目标：10 倍差距
- 100 kHz 验证 → 1 Msps 目标：10 倍差距
- **总差距：100 倍！**

#### 测试计划

**阶段 1：中频验证（50-100 kHz）**
```
测试频率：50 kHz, 75 kHz, 100 kHz
测试方法：
  1. 使用 freq_measure_cpp.cpp 测量准确度
  2. 记录边沿丢失率
  3. 测量 CPU 使用率
  4. 验证 4 通道同时采集
